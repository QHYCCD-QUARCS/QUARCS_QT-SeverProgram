...
// 回调函数类型定义
using ImageReceivedCallback = std::function<void(const std::string &filename,
...
public:
    ...
    uint32_t getCCDFrameInfo(INDI::BaseDevice *dp, int &X, int &Y, int &WIDTH,
                             int &HEIGHT);
    uint32_t setCCDFrameInfo(INDI::BaseDevice *dp, int X, int Y, int WIDTH,
                             int HEIGHT);  // something like setROI
    uint32_t resetCCDFrameInfo(
        INDI::BaseDevice
            *dp);  // will reset the ROI to default and reset the BIN to bin11
    uint32_t setCCDCooler(INDI::BaseDevice *dp, bool enable);
    uint32_t getCCDCooler(INDI::BaseDevice *dp, bool &enable);
    uint32_t getCCDBasicInfo(INDI::BaseDevice *dp, int &maxX, int &max,
                             double &pixelsize, double &pixelsizX,
                             double &pixelsizY, int &bitDepth);
    uint32_t getCCDBinning(INDI::BaseDevice *dp, int &BINX, int &BINY,
                           int &BINXMAX, int &BINYMAX);
    uint32_t setCCDBinnign(INDI::BaseDevice *dp, int BINX, int BINY);
    uint32_t getCCDCFA(INDI::BaseDevice *dp, int &offsetX, int &offsetY,
                       QString &CFATYPE);
    ...
    uint32_t getCCDReadMode(INDI::BaseDevice *dp, int &value, int &min,
                            int &max);
    uint32_t setCCDReadMode(INDI::BaseDevice *dp, int value);

    // Telescope API
    uint32_t getTelescopeInfo(INDI::BaseDevice *dp, double &telescope_aperture,
                              double &telescope_focal, double &guider_aperature,
                              double &guider_focal);
    uint32_t setTelescopeInfo(INDI::BaseDevice *dp, double telescope_aperture,
                              double telescope_focal, double guider_aperature,
                              double guider_focal);
    ...
    uint32_t getTelescopeParkPosition(INDI::BaseDevice *dp, double &RA,
                                      double &DEC);
    uint32_t setTelescopeParkPosition(INDI::BaseDevice *dp, double RA,
                                      double DEC);
    ...
    uint32_t getTelescopeMaxSlewRateOptions(INDI::BaseDevice *dp, int &min,
                                            int &max, int &value);
    ...
    uint32_t setTelescopeActionAfterPositionSet(INDI::BaseDevice *dp,
                                                QString action);
    uint32_t getTelescopeRADECJ2000(INDI::BaseDevice *dp, double &RA_Hours,
                                    double &DEC_Degree);
    uint32_t setTelescopeRADECJ2000(INDI::BaseDevice *dp, double RA_Hours,
                                    double DEC_Degree);
    uint32_t getTelescopeRADECJNOW(INDI::BaseDevice *dp, double &RA_Hours,
                                   double &DEC_Degree);
    uint32_t setTelescopeRADECJNOW(INDI::BaseDevice *dp, double RA_Hours,
                                   double DEC_Degree,
                                   INDI::PropertyNumber &property);
    uint32_t getTelescopeTargetRADECJNOW(INDI::BaseDevice *dp, double &RA_Hours,
                                         double &DEC_Degree);
    uint32_t setTelescopeTargetRADECJNOW(INDI::BaseDevice *dp, double RA_Hours,
                                         double DEC_Degree);
    uint32_t getTelescopetAZALT(INDI::BaseDevice *dp, double &AZ_DEGREE,
                                double &ALT_DEGREE);
    uint32_t setTelescopetAZALT(INDI::BaseDevice *dp, double AZ_DEGREE,
                                double ALT_DEGREE);

    uint32_t slewTelescopeJNowNonBlock(INDI::BaseDevice *dp, double RA_Hours,
                                       double DEC_Degree, bool EnableTracking,
                                       INDI::PropertyNumber &property);
    uint32_t syncTelescopeJNow(INDI::BaseDevice *dp, double RA_Hours,
                               double DEC_Degree,
                               INDI::PropertyNumber &property);
    ...
    //--------------------CFW API
    uint32_t getCFWPosition(INDI::BaseDevice *dp, int &position, int &min,
                            int &max);
    ...
    // Focuser API
    uint32_t getFocuserSpeed(INDI::BaseDevice *dp, int &value, int &min,
                             int &max);
    ...
    //-----------GENERIC API-------------
    uint32_t getDevicePort(INDI::BaseDevice *dp,
                           QString &Device_Port);  // add by CJQ 2023.3.3
    uint32_t setDevicePort(INDI::BaseDevice *dp,
                           QString Device_Port);  // add by CJQ 2023.3.22
    uint32_t setTimeUTC(INDI::BaseDevice *dp, QDateTime datetime);
    uint32_t getTimeUTC(INDI::BaseDevice *dp, QDateTime &datetime);
    uint32_t setLocation(INDI::BaseDevice *dp, double latitude_degree,
                         double longitude_degree, double elevation);
    uint32_t getLocation(INDI::BaseDevice *dp, double &latitude_degree,
                         double &longitude_degree, double &elevation);
    uint32_t setAtmosphere(INDI::BaseDevice *dp, double temperature,
                           double pressure, double humidity);
    uint32_t getAtmosphere(INDI::BaseDevice *dp, double &temperature,
                           double &pressure, double &humidity);
    ...
    // signals:
    //     void indiBlobImageReceived(QString filename, QString devname);  //
    //     Q_SIGNAL
