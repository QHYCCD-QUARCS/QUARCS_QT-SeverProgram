                    ...
                    while (!(xmlReader.isEndElement() &&
                        ...
                        if (xmlReader.isStartElement()) {
                            if (xmlReader.name() == "Description") {
                                ...
                                systemDevice.DeviceIndiName =
                                    xmlReader.readElementText();
                            } else if (xmlReader.name() == "DriverIndiName") {
                                systemDevice.DriverIndiName =
                                    xmlReader.readElementText();
                            } else if (xmlReader.name() == "DriverFrom") {
                                systemDevice.DriverFrom =
                                    xmlReader.readElementText();
                            }
                        }
                        xmlReader.readNext();
...
void Tools::SaveSystemListToXml(const QString& fileName) {
    ...
    if (file.open(QIODevice::WriteOnly)) {
        ...
        for (const auto& systemDevice : systemDeviceList_.system_devices) {
            ...
            xmlWriter.writeTextElement("DeviceIndiName",
                                       systemDevice.DeviceIndiName);
            xmlWriter.writeTextElement("DriverIndiName",
                                       systemDevice.DriverIndiName);
...
void Tools::readDriversListFromFiles(const std::string& filename,
    ...
    while (!xml.atEnd() && !xml.hasError()) {
        ...
        if (xml.isStartElement() && xml.name() == "devGroup") {
            DevGroup dev_group;
            // dev_group.group =
            // xml.attributes().value("group").toString().toStdString();
            dev_group.group =
...
void Tools::printDevGroups2(const DriversList driver_list) {
    ...
    for (int i = 0; i < driver_list.dev_groups.size(); i++) {
        ...
        // qDebug() << "Number of devices:" <<
        // driver_list.dev_groups[i].devices.size();
        for (int j = 0; j < driver_list.dev_groups[i].devices.size(); j++) {
            qDebug() << QString::fromStdString(
                            driver_list.dev_groups[i].devices[j].driver_name)
                     << QString::fromStdString(
                            driver_list.dev_groups[i].devices[j].version)
                     << QString::fromStdString(
                            driver_list.dev_groups[i].devices[j].label);
        }
    }
}
...
void Tools::printSystemDeviceList(SystemDeviceList s) {
    ...
    qDebug() << "*****************System Device Selected***************"
             << s.system_devices.size();
    QString dpName;
    for (int i = 0; i < s.system_devices.size(); i++) {
        if (s.system_devices[i].dp == NULL)
            dpName = "NULL";
        else
            dpName = s.system_devices[i].dp->getDeviceName();

        qDebug() << i << s.system_devices[i].DeviceIndiGroup
                 << s.system_devices[i].DriverFrom
                 << s.system_devices[i].DriverIndiName
                 << s.system_devices[i].DeviceIndiName
                 << s.system_devices[i].Description
                 << s.system_devices[i].isConnect << dpName;
    }
    qDebug() << "******************************************************";
}
    ...
    if (!std::filesystem::exists(directory)) {
        if (std::filesystem::create_directory(directory)) {
            ...
            if (std::filesystem::create_directory(captureDirectory)) {
                std::cout << "子文件夹创建成功: " << captureDirectory
                          << std::endl;
            } else {
            ...
            if (std::filesystem::create_directory(scheduleDirectory)) {
                std::cout << "子文件夹创建成功: " << scheduleDirectory
                          << std::endl;
            } else {
                std::cerr << "创建子文件夹时发生错误" << std::endl;
            }
...
void Tools::saveSystemDeviceList(SystemDeviceList deviceList) {
    ...
    for (const auto& device : deviceList.system_devices) {
        ...
        size_t deviceIndiNameSize =
            static_cast<size_t>(deviceIndiNameUtf8.size());
        ...
        size_t driverIndiNameSize =
            static_cast<size_t>(driverIndiNameUtf8.size());
...
SystemDeviceList Tools::readSystemDeviceList() {
    ...
    while (true) {
        ...
        device.Description = QString::fromUtf8(
            descriptionBuffer.data(), static_cast<int>(descriptionSize));
        ...
        device.DeviceIndiName = QString::fromUtf8(
            deviceIndiNameBuffer.data(), static_cast<int>(deviceIndiNameSize));
        ...
        device.DriverIndiName = QString::fromUtf8(
            driverIndiNameBuffer.data(), static_cast<int>(driverIndiNameSize));
        ...
        device.DriverFrom = QString::fromUtf8(driverFromBuffer.data(),
                                              static_cast<int>(driverFromSize));
...
void Tools::saveExpTimeList(QString List) {
    std::string directory = "config";  // 配置文件夹名
    std::string filename =
        directory + "/ExpTimeList.dat";  // 在配置文件夹中创建文件
    ...
    outfile.write(reinterpret_cast<const char*>(&ExpTimeListSize),
                  sizeof(size_t));
...
void Tools::saveCFWList(QString Name, QString List) {
    std::string directory = "config";  // 配置文件夹名
    std::string filename = directory + "/CFWList(" + Name.toStdString() +
                           ").dat";  // 在配置文件夹中创建文件
...
void Tools::initSystemDeviceList(SystemDeviceList& s) {
    ...
    dev.DeviceIndiName = "";
    dev.DriverFrom =
        "";  // DriverFrom 用于存储驱动类型。如果来自于INDI，则是“INDI"
             // 如果来自于QHYCCD SDK  则是”QHYCCDSDK"
    dev.isConnect = false;
    dev.dp = NULL;
...
uint32_t Tools::getIndexFromSystemDeviceList(SystemDeviceList s,
                                             QString devname, int& index) {
    int i = 0;
    for (auto dev : s.system_devices) {
        if (dev.DeviceIndiName == devname) {
            index = i;
    ...
    if (i < 32) {
        index = i;
        qDebug() << "getIndexFromSystemDeviceList | found device in system "
                    "list. device name"
                 << devname << "index" << index;
...
void Tools::stopIndiDriverAll(const DriversList driver_list) {
    ...
    // under operaiton, stop it may cause crash
    ...
    for (int i = 0; i < driver_list.dev_groups.size(); i++) {
        // qDebug() << "------" <<
        // QString::fromStdString(driver_list.dev_groups[i].group) << "------";
        for (int j = 0; j < driver_list.dev_groups[i].devices.size(); j++) {
            stopIndiDriver(QString::fromStdString(
                driver_list.dev_groups[i].devices[j].driver_name));
        }
    }
}
...
uint32_t Tools::readFitsHeadForDevName(std::string filename, QString& devname) {
    // this api is to readout the devname in the FITs . Can used to identity the
    // image for the newBLOB
    ...
    for (int i = 1; i <= nkeys; i++) {  // Read and print each keyword
...
int Tools::readFits(const char* fileName, cv::Mat& image) {
    ...
    if (fits_read_img(fptr, TUSHORT, 1, nelements, NULL, array, NULL,
                      &status)) {
        delete[] array;
        return status;
    }
    ...
    // 将数据转换为 cv::Mat
    if (bitpix == 16)
        image = cv::Mat(naxes[1], naxes[0], CV_16U, array).clone();
    else if (bitpix == 8)
        image = cv::Mat(naxes[1], naxes[0], CV_8U, array).clone();
...
void Tools::ScanCamera() {
    if (Tools::systemDeviceList().currentDeviceCode != 1) {
        ...
        if (QHYCCD_SUCCESS == ret) {
            ...
            qDebug() << "SetQHYCCDStreamMode:" << mode
                     << "failure, error:" << ret;
        }
        qDebug() << "\033[0m\033[1;35m" << "initialize camera" << "\033[0m";
        // initialize camera
...
cv::Mat Tools::Capture() {
    ...
    ret = GetQHYCCDChipInfo(camhandle_, &chipWidthMM, &chipHeightMM,
                            &maxImageSizeX, &maxImageSizeY, &pixelWidthUM,
                            &pixelHeightUM, &bpp);
    if (QHYCCD_SUCCESS == ret) {
    ...
    if (QHYCCD_SUCCESS == ret) {
        ...
        if (QHYCCD_SUCCESS == ret) {
            qDebug() << "SetQHYCCDParam CONTROL_USBTRAFFIC set to:"
                     << USB_TRAFFIC << "success.";
        } else {
    ...
    ret = SetQHYCCDResolution(camhandle_, roiStartX, roiStartY, roiSizeX,
                              roiSizeY);
    if (QHYCCD_SUCCESS == ret) {
    ...
    ret = GetQHYCCDSingleFrame(camhandle_, &roiSizeX, &roiSizeY, &bpp,
                               &channels, pImgData);
    if (QHYCCD_SUCCESS == ret) {
...
bool Tools::WriteFPGA(uint8_t hand, int command) {
    if (hand == 0xa0) {
        ...
        switch (dir) {
            case 1: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0xa0, 0x01) ==
                       QHYCCD_SUCCESS;
            }
            case 2: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0xa0, 0x02) ==
                       QHYCCD_SUCCESS;
            }
            case 3: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0xa0, 0x03) ==
                       QHYCCD_SUCCESS;
            }
            case 4: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0xa0, 0x04) ==
                       QHYCCD_SUCCESS;
            }
            default:
                break;
    ...
    if (hand == 0xa1) {
        ...
        switch (dir) {
            case 1: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0xa1, 0x01) ==
                       QHYCCD_SUCCESS;
            }
            case 0: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0xa1, 0x00) ==
                       QHYCCD_SUCCESS;
            }
            default:
                break;
    ...
    if (hand == 0xa2) {
        ...
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa2, m_com) ==
                      QHYCCD_SUCCESS);

        m_com = qq % (256 * 256 * 256 * 256 * 256 * 256 * 256) /
                (256 * 256 * 256 * 256 * 256 * 256);
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa3, m_com) ==
                      QHYCCD_SUCCESS);

        m_com = qq % (256 * 256 * 256 * 256 * 256 * 256 * 256) %
                (256 * 256 * 256 * 256 * 256 * 256) /
                (256 * 256 * 256 * 256 * 256);
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa4, m_com) ==
                      QHYCCD_SUCCESS);

        m_com = qq % (256 * 256 * 256 * 256 * 256 * 256 * 256) %
                (256 * 256 * 256 * 256 * 256 * 256) %
                (256 * 256 * 256 * 256 * 256) / (256 * 256 * 256 * 256);
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa5, m_com) ==
                      QHYCCD_SUCCESS);
        ...
        m_com = qq % (256 * 256 * 256 * 256 * 256 * 256 * 256) %
                ...
                (256 * 256 * 256 * 256 * 256) % (256 * 256 * 256 * 256) /
                (256 * 256 * 256);
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa6, m_com) ==
                      QHYCCD_SUCCESS);
        ...
        m_com = qq % (256 * 256 * 256 * 256 * 256 * 256 * 256) %
                ...
                (256 * 256 * 256 * 256 * 256) % (256 * 256 * 256 * 256) %
                (256 * 256 * 256) / (256 * 256);
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa7, m_com) ==
                      QHYCCD_SUCCESS);
        ...
        m_com = qq % (256 * 256 * 256 * 256 * 256 * 256 * 256) %
                ...
                (256 * 256 * 256 * 256 * 256) % (256 * 256 * 256 * 256) %
                (256 * 256 * 256) % (256 * 256) / 256;
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa8, m_com) ==
                      QHYCCD_SUCCESS);
        ...
        m_com = qq % (56 * 256 * 256 * 256 * 256 * 256 * 256) %
                ...
                (256 * 256 * 256 * 256 * 256) % (256 * 256 * 256 * 256) %
                (256 * 256 * 256) % (256 * 256) % 256;
        ret = ret && (SetQHYCCDWriteFPGA(camhandle_, 0, 0xa9, m_com) ==
                      QHYCCD_SUCCESS);
    ...
    if (hand == 0xa4) {
        ...
        switch (dir) {
            case 1: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0x9f, 0x01) ==
                       QHYCCD_SUCCESS;
            }
            case 0: {
                return SetQHYCCDWriteFPGA(camhandle_, 0, 0x9f, 0x00) ==
                       QHYCCD_SUCCESS;
            }
            default:
                break;
...
// 将输入图像分成三个通道
    ...
    if (image.channels() == 3) {
        ...
        cv::calcHist(&channels[0], 1, 0, cv::Mat(), b_hist, 1, &bins,
                     &histRange);
        cv::calcHist(&channels[1], 1, 0, cv::Mat(), g_hist, 1, &bins,
                     &histRange);
        cv::calcHist(&channels[2], 1, 0, cv::Mat(), r_hist, 1, &bins,
                     &histRange);
        ...
        // for color image. need to use the global min_value as the same
        // threshold for three channel.
        int min_maxval = std::min({max_b, max_g, max_r});
#ifdef ImageDebug
        qDebug()
            << "showHistogram | Draw Histograme color | min of max value in 3 "
               "channels"
            << min_maxval;
#endif
        // qDebug()<<min_maxval;
        ...
        for (int i = 1; i < bins; i++) {
            ...
            cv::line(
                histImage,
                cv::Point(bin_w * (i - 1),
                          height - cvRound(g_hist.at<float>(i - 1))),
                cv::Point(bin_w * (i), height - cvRound(g_hist.at<float>(i))),
                cv::Scalar(0, 255, 0));
            cv::line(
                histImage,
                cv::Point(bin_w * (i - 1),
                          height - cvRound(r_hist.at<float>(i - 1))),
        ...
        ret = QImage((unsigned char*)histImage.data, histImage.cols,
        ...
        QImage qimage(histImage.data, histImage.cols, histImage.rows,
        ...
        for (int i = 1; i < bins; i++) {
            cv::line(
                histImage,
                cv::Point(bin_w * (i - 1),
                          height - cvRound(hist.at<float>(i - 1))),
...
void Tools::PaintHistogram(cv::Mat src, QLabel* label) {
    ...
    cv::calcHist(&gray, 1, channels, cv::Mat(), hist, dims, histSize_gray,
                 ranges, true, false);  // 计算直方图
    ...
    if (channelCount == 3) {
        ...
        int min_maxval = std::min({max_b, max_g, max_r});
#ifdef ImageDebug
        qDebug() << "showHistogram | Draw Histograme color | min of max value "
                    "in 3 channels"
                 << min_maxval;
#endif
        // qDebug()<<min_maxval;
    ...
    cv::normalize(hist, hist, 0, histImage.rows, cv::NORM_MINMAX, -1,
                  cv::Mat());  // 将直方图归一化到[0,histImage.rows]
...
void Tools::AutoStretch(cv::Mat src) {
    ...
    if (channelCount == 3) {
        ...
        for (int i = 0; i < channels.size(); ++i) {
            ...
            channels[i].convertTo(channels[i], CV_8U, 255.0 / (maxVal - minVal),
        ...
        cv::Mat stretchedImage;
        src.convertTo(stretchedImage, CV_8U, 255.0 / (maxVal - minVal),
                      -minVal * 255.0 / (maxVal - minVal));
    }
}
...
void Tools::ShowCvImageOnQLabel(cv::Mat image, QLabel* label) {
    ...
    QImage qtImage = QImage((const unsigned char*)(image.data), image.cols,
...
void Tools::ShowOpenCV_QLabel_withRotate(cv::Mat img, QLabel* label,
                                         int RotateType,
                                         double cmdRotate_Degree,
                                         double AzAltToRADEC_Degree) {
    ...
    h = img.rows;
#ifdef ImageDebug
    qDebug() << "showOpenCV_QLabel_withRotate | " << w << h << img.channels()
             << img.type();
#endif
    cv::Mat imgRGB888;
    ...
    // circle(imgRGB888,cv::Point(w/2,w/2),10,CV_RGB(0, 0, 255),1,8,0);  //make
    // a mark in center point of the image

    unsigned int byteCount;
    byteCount = w * h * 3;  // 3 channel 8bit image
    QByteArray p_w_picpathByteArray =
        QByteArray((const char*)imgRGB888.data, byteCount);
    ...
    // rotate:  180-cmdRoate_Degree will rotate to the RADEC system.  -
    // RadToDegree(glRotateDataAzAltToRADEC_radian) will rotate from RADEC to
    // AZALT system
    int nDegree;
    ...
    if (RotateType == 0)
        ...
        nDegree = 180 - cmdRotate_Degree;
    else if (RotateType == 2)
        nDegree = 180 - cmdRotate_Degree -
                  AzAltToRADEC_Degree;  // cmdRotate_Degree-180;
...
#ifdef ImageDebug
    qDebug() << "getAutoStretch |mean std B W" << mean.val[0] << std.val[0] << B
             << W;
...
// this API support 16bit image input, 3 channel and 1 channel
    ...
    if (img16.channels() == 1) {
        ...
        */
        ...
        // this code will take only 50ms for 6000*4000 color image (under
        // release).
        uint16_t* data16;
        ...
        // The following code will take about 400ms for 6000*4000 color image
        // (under release)
        /*
...
double Tools::getDecAngle(const QString& str) {
    ...
    if (rex.exactMatch(str)) {
        ...
        double s = 0;
        ushort hd =
            caps.at(5).isEmpty() ? 'd' : caps.at(5).toLower().at(0).unicode();
...
// bool Tools::DetectStar(cv::Mat image, double threshold, int minArea,
// cv::Rect& starRect)
// {
...
QList<FITSImage::Star> Tools::FindStarsByStellarSolver(bool AllStars,
                                                       bool runHFR) {
    Tools tempTool;
    ...
    stars = tempTool.FindStarsByStellarSolver_(AllStars, imageStats,
                                               imageBuffer, runHFR);
    return stars;
}

QList<FITSImage::Star> Tools::FindStarsByStellarSolver_(
    bool AllStars, const FITSImage::Statistic& imagestats,
    ...
    for (const auto& star : stars) {
        std::cout << "Star at (" << star.x << ", " << star.y
                  << ") with HFR: " << star.HFR << std::endl;
    }
...
loadFitsResult Tools::loadFits(QString fileName) {
    ...
    // Use open diskfile as it does not use extended file names which has
    // problems opening files with [ ] or ( ) in their names.
    ...
    int fitsBitPix = 0;
    if (fits_get_img_param(fptr, 3, &fitsBitPix, &(stats.ndim), naxes,
                           &status)) {
    ...
    switch (fitsBitPix) {
        ...
        case DOUBLE_IMG:
            ...
            break;
        default:
            // logIssue(QString("Bit depth %1 is not
            // supported.").arg(fitsBitPix));
    ...
    if (naxes[0] == 0 || naxes[1] == 0) {
        // logIssue(QString("Image has invalid dimensions
        // %1x%2").arg(naxes[0]).arg(naxes[1]));
    ...
    m_ImageBufferSize = stats.samples_per_channel * stats.channels *
                        static_cast<uint16_t>(stats.bytesPerPixel);
    // deleteImageBuffer();
    if (m_ImageBuffer) {
    ...
    if (m_ImageBuffer == nullptr) {
        // logIssue(QString("FITSData: Not enough memory for image_buffer
        // channel. Requested: %1 bytes ").arg(m_ImageBufferSize));
        qDebug() << "FITSData: Not enough memory for image_buffer channel. "
                    "Requested:"
    ...
    if (fits_read_img(fptr, static_cast<uint16_t>(stats.dataType), 1, nelements,
                      nullptr, m_ImageBuffer, &anynullptr, &status)) {
...
FWHM_Result Tools::CalculateFWHM(cv::Mat image) {
    ...
    resize(subimage, subimage, cv::Size(), scale_up, scale_up,
           cv::INTER_LINEAR);
    ...
    qDebug() << "Barycentric Coordinate:" << FirstMoment_x << ","
             << FirstMoment_y;
...
HFR_Result Tools::CalculateHFR(cv::Mat image) {
    ...
    double scale_up = 10.0;
    cv::resize(subimage, subimage, cv::Size(), scale_up, scale_up,
               cv::INTER_LINEAR);
    ...
    for (int y = 0; y < subimage.rows; ++y) {
        for (int x = 0; x < subimage.cols; ++x) {
            ...
            if (pixelValue >= halfMaxBrightness) {
                double distance = std::sqrt(std::pow(x - FirstMoment_x, 2) +
                                            std::pow(y - FirstMoment_y, 2));
    ...
    // 在原图上绘制检测结果
    cv::Mat imagePoint = image.clone();
    cv::Point center(starRect.x + FirstMoment_x / 10,
                     starRect.y + FirstMoment_y / 10);
    cv::rectangle(imagePoint, starRect, cv::Scalar(0, 255, 0),
                  1);  // 绘制外接矩形
    cv::circle(imagePoint, center, static_cast<int>(HFR), cv::Scalar(0, 0, 255),
               1);  // 绘制HFR圆
    cv::circle(imagePoint, center, 1, cv::Scalar(0, 255, 0), -1);  // 绘制中心点

    // 在图像上显示HFR数值
    std::string hfrText = cv::format("%.2f", HFR);
    cv::putText(imagePoint, hfrText, cv::Point(starRect.x, starRect.y - 5),
...
// #ifdef debug
//   qDebug("tools.cpp|getLST_Degree|longitude (degree) = %f",
//   longitude_Degree);
// #endif
...
// #ifdef debug
//   qDebug("tools.cpp|getLST_Degree|LST after  range (degree) %f (hms) %s",
//   LST,
...
//                                   double longitude_radian, double&
//                                   alt_radian, double& az_radian) {
    ...
    if (cos_lat < .00001) /* polar case */
        ...
        temp = acos(
            fminl(fmaxl(temp, -1.0),
                  1.0));  // to resolve the return value = nan when alt=90degree
                          // or -90degree. , before it is temp= acos(-temp);
...
QString Tools::getInfoTextA(QDateTime T_local, double RA_DEGREE,
    ...
    strs[3] = MOUNTSTATU;
    strs[4] = GUIDESTATU;
    strs[5] = "RMS " + QString::number(dRA_degree) + "/" +
              QString::number(dDEC_degree);
...
QString Tools::getInfoTextB(QDateTime T_utc, double AZ_rad, double ALT_rad,
    ...
    strs[1] = "AZ/ALT";
    // 2023.12.21 CJQ
    //   strs[2] = StelUtils::radToDmsStr(AZ_rad) + " " +
    //   StelUtils::radToDmsStr(ALT_rad);
    strs[3] = CAMSTATU;
...
CartesianCoordinates Tools::convertEquatorialToCartesian(double ra, double dec,
                                                         double radius) {
    // 将赤经和赤纬转换为弧度
...
CartesianCoordinates Tools::calculateVector(CartesianCoordinates pointA,
                                            CartesianCoordinates pointB) {
...
CartesianCoordinates Tools::calculatePointC(CartesianCoordinates pointA,
                                            CartesianCoordinates vectorV) {
...
SphericalCoordinates Tools::convertToSphericalCoordinates(
    CartesianCoordinates cartesianPoint) {
...
MinMaxFOV Tools::calculateFOV(int FocalLength, double CameraSize_width,
                              double CameraSize_height) {
    MinMaxFOV result;
    qDebug() << "FocalLength: " << FocalLength << ", "
             << "CameraSize: " << CameraSize_width << ", " << CameraSize_height;

    double CameraSize_diagonal =
        sqrt(pow(CameraSize_width, 2) + pow(CameraSize_height, 2));
    ...
    qDebug() << "minFov: " << result.minFOV << ", "
             << "maxFov: " << result.maxFOV;

    return result;
}

SloveResults Tools::PlateSlove(int FocalLength, double CameraSize_width,
                               double CameraSize_height, double Ra_Degree,
                               double Dec_Degree, bool USEQHYCCDSDK) {
    SloveResults result;
    MinMaxFOV FOV;

    // qDebug() << "Ra_Degree:" << Ra_Degree << "," << "Dec_Degree:" <<
    // Dec_Degree;
    ...
    if (USEQHYCCDSDK == false) {
        ...
        // command_qstr="solve-field " + filename + ".fits" + " --overwrite
        // --scale-units degwidth --scale-low " + MinFOV + " --scale-high " +
        // MaxFOV + " --ra " + RA + " --dec " + DEC + " --radius 10 --nsigma 12
        // --no-plots  --no-remove-lines --uniformize 0 --timestamp";
        command_qstr = "solve-field " + filename + ".fits" +
                       " --overwrite --scale-units degwidth --scale-low " +
                       MinFOV + " --scale-high " + MaxFOV +
                       " --radius 10 --nsigma 12  --no-plots  "
                       "--no-remove-lines --uniformize 0 --timestamp";
    } else {
        filename = "/dev/shm/SDK_Capture";
        command_qstr = "solve-field " + filename + ".png" +
                       " --overwrite --scale-units degwidth --scale-low " +
                       MinFOV + " --scale-high " + MaxFOV + " --ra " + RA +
                       " --dec " + DEC +
                       " --radius 10 --nsigma 12  --no-plots  "
                       "--no-remove-lines --uniformize 0 --timestamp";
    }
    ...
    qDebug("RA DEC Rotation(degree) %f %f %f", RA_Degree, DEC_Degree,
           Rotation_Degree);
    if (str == "") {
        ...
        qDebug() << "RA DEC " << QString::number(RA_Degree, 'g', 9) << " "
                 << QString::number(DEC_Degree, 'g', 9);
...
int Tools::fitQuadraticCurve(const QVector<QPointF>& data, float& a, float& b,
                             float& c) {
    int n = data.size();
    if (n < 5) {
        return -1;  // 数据点数量不足
...
double Tools::calculateRSquared(QVector<QPointF> data, float a, float b,
                                float c) {
    double ssTotal = 0.0;
...
// 		if (newSelected[0]->getEnglishName()!=
...
// 			Search_mvmgr->moveToObject(newSelected[0],
// Search_mvmgr->getAutoMoveDuration());
