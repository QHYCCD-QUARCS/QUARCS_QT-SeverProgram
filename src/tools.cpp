...
bool Tools::PlateSolve(QString filename, int FocalLength, double CameraSize_width, double CameraSize_height, bool USEQHYCCDSDK, int mode, double lastRA, double lastDEC, double searchRadiusDeg, const QString &backendConfigPath, int solveTimeoutMs)
{
    ...
    const QString solveFieldProgram = resolveAstrometryExecutable(QStringLiteral("solve-field"));

    QProcess* cmd_test = new QProcess();
    applyAstrometryEnvironment(*cmd_test, solveFieldProgram);
    ...
    command_qstr.replace(QStringLiteral("solve-field "), solveFieldProgram + QStringLiteral(" "));
    ...
    if (!cmd_test->waitForStarted()) {
        Logger::Log("解析命令启动失败: " + cmd_test->errorString().toStdString(), LogLevel::ERROR, DeviceType::MAIN);
    ...
    if (exitCode != 0) {
        const QString err = QString::fromLocal8Bit(cmd_test->readAllStandardError()).trimmed();
        if (!err.isEmpty()) {
            Logger::Log("解析命令stderr: " + err.toStdString(), LogLevel::ERROR, DeviceType::MAIN);
        }
...
SloveResults Tools::ReadSolveResult(QString filename, int imageWidth, int imageHeight) {
  ...
  const QString wcsinfoProgram = resolveAstrometryExecutable(QStringLiteral("wcsinfo"));
  applyAstrometryEnvironment(*cmd_test, wcsinfoProgram);
  Logger::Log("执行wcsinfo: " + wcsinfoProgram.toStdString() + " " + (filename + QStringLiteral(".wcs")).toStdString(),
              LogLevel::INFO,
              DeviceType::MAIN);
  cmd_test->start(wcsinfoProgram, {filename + QStringLiteral(".wcs")});
  cmd_test->waitForFinished();  // 等待外部程序执行完成

  QString str = cmd_test->readAllStandardOutput();  // 读取程序输出的结果
  const QString err = QString::fromLocal8Bit(cmd_test->readAllStandardError()).trimmed();
  if (!err.isEmpty()) {
    Logger::Log("wcsinfo stderr: " + err.toStdString(), LogLevel::WARNING, DeviceType::MAIN);
  }
...
SphericalCoordinates Tools::xy2rdByExternal(const QString& wcsFile, double x, double y, bool& ok) {
    ok = false;
    QProcess proc;
    const QString wcsXy2rdProgram = resolveAstrometryExecutable(QStringLiteral("wcs-xy2rd"));
    applyAstrometryEnvironment(proc, wcsXy2rdProgram);
    QString cmd = QString("%1 -w %2 -x %3 -y %4")
                      .arg(wcsXy2rdProgram)
                      .arg(wcsFile)
                      .arg(x, 0, 'f', 6)
                      .arg(y, 0, 'f', 6);
    proc.start(wcsXy2rdProgram,
