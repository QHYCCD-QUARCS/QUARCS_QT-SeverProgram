...
struct DriversList {
    ...
    // this value is used in the selector code as a global value to indicate
    // which grounp is selected
...
struct SystemDevice {
    QString Description{};
    int DeviceIndiGroup{};     // number or text.  if number, using the INDI
                               // grounp// number standard
...
public:
    ...
    static bool GetIndexFromSystemDeviceList(const QString& devname,
                                             int& index);
    static void ClearSystemDeviceListItem(int index);
    static SystemDeviceList& systemDeviceList();

    static void readDriversListFromFiles(const std::string& filename,
    ...
    static uint32_t readFitsHeadForDevName(std::string filename,
                                           QString& devname);
    ...
    static uint32_t getIndexFromSystemDeviceList(SystemDeviceList s,
                                                 QString devname,
                                                 int& index);  //
    ...
    static void ShowOpenCV_QLabel_withRotate(cv::Mat img, QLabel* label,
                                             int RotateType,
    ...
    // static bool DetectStar(cv::Mat image, double threshold, int minArea,
    // cv::Rect& starRect);
    ...
    static QList<FITSImage::Star> FindStarsByStellarSolver(bool AllStars,
                                                           bool runHFR);
    ...
    static double getCurrentMeridanRA(double Latitude_radian,
                                      double Longitude_radian);
    static bool periodBelongs(double value, double min, double max,
                              double period, bool minequ, bool maxequ);
    ...
    // static void MRGOTO_RADEC_rad(StelMovementMgr* mvmgr, double RA, double
    // DEC); static void MRGOTO_AZALT_rad(StelMovementMgr* mvmgr, double AZ,
    // double ALT);
    static CartesianCoordinates convertEquatorialToCartesian(double ra,
                                                             double dec,
                                                             double radius);
    static CartesianCoordinates calculateVector(CartesianCoordinates pointA,
                                                CartesianCoordinates pointB);
    static CartesianCoordinates calculatePointC(CartesianCoordinates pointA,
                                                CartesianCoordinates vectorV);
    static SphericalCoordinates convertToSphericalCoordinates(
        CartesianCoordinates cartesianPoint);

    static MinMaxFOV calculateFOV(int FocalLength, double CameraSize_width,
                                  double CameraSize_height);
    static SloveResults PlateSlove(int FocalLength, double CameraSize_width,
    ...
    static int fitQuadraticCurve(const QVector<QPointF>& data, float& a,
                                 float& b, float& c);

    static double calculateRSquared(QVector<QPointF> data, float a, float b,
                                    float c);
...
private:
    ...
    QList<FITSImage::Star> FindStarsByStellarSolver_(
