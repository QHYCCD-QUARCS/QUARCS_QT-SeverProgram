...
MyClient::MyClient() {
    ...
    qDebug() << "MyClient::MyClent" << gethostid() << getPort() << getegid()
             << geteuid();
}
...
void MyClient::newProperty(INDI::Property property) {
    ...
    // qDebug("Recveing message from Server %s",
    // baseDevice.messageQueue(messageID).c_str());
}

void MyClient::newDevice(INDI::BaseDevice baseDevice) {
    // the new Device is a callback function , the connect server will trig it.
    // it can be override here
...
void MyClient::updateProperty(INDI::Property property) {
    if (property.getType() == INDI_BLOB) {
        ...
        qDebug() << "\033[32m" << "Exposure completed:" << CaptureTestTime
                 << "milliseconds" << "\033[0m";
        CaptureTestTimer.invalidate();

        qDebug() << "newProperty(getName): " << property.getName();
        qDebug(
            "Recveing image from Server size len name label format %d %d %s %s "
            "%s",
            property.getBLOB()->bp->size, property.getBLOB()->bp->bloblen,
            property.getBLOB()->bp->name, property.getBLOB()->bp->label,
            property.getBLOB()->bp->format);
        ...
        myfile.write(static_cast<char *>(property.getBLOB()->bp->blob),
                     property.getBLOB()->bp->bloblen);
        myfile.close();

        CaptureTestTime = CaptureTestTimer.elapsed();
        qDebug() << "\033[32m"
                 << "Save ccd_simulator.fits completed:" << CaptureTestTime
                 << "milliseconds" << "\033[0m";
        CaptureTestTimer.invalidate();
...
int MyClient::GetDeviceCount() const { return deviceList.size(); }
...
QString MyClient::PrintDevices() {
    ...
    if (deviceNames.size() == 0) {
        ...
        for (int i = 0; i < deviceNames.size(); i++) {
            qDebug() << i << deviceNames[i].c_str()
                     << deviceList[i]->getDriverExec();
            if (i > 0) {
                dev.append("|");  // 添加分隔符
            }
...
INDI::BaseDevice *MyClient::GetDeviceFromList(int index) {
    // 这个函数接受一个整型参数 index，表示要返回的设备在列表中的位置。如果
    // index 超出了列表的范围，函数返回 nullptr。否则，函数返回 deviceList
    // 数组中对应位置的设备指针。
...
//               qDebug()<<"disconnectAllDevice
//               |"<<dp->getDeviceName()<<dp->isConnected();
//           }
//     }
// }
...
void MyClient::disconnectAllDevice(void) {
    ...
    for (int i = 0; i < GetDeviceCount(); i++) {
        ...
        qDebug() << "disconnectAllDevice |" << dp[i]->getDeviceName()
                 << dp[i]->isConnected();
    }
}

// need to wait the connection completely finished then call this , otherwise it
// may not output all
void MyClient::listAllProperties(INDI::BaseDevice *dp) {
    std::vector<INDI::Property> properties(dp->getProperties().begin(),
                                           dp->getProperties().end());

    // Iterate over the list of properties and print the names of the
    // PropertyNumber properties
    for (INDI::Property *property : properties) {
        // INDI::PropertyNumber *numberProperty =
        // static_cast<INDI::PropertyNumber *>(property);
        if (property != nullptr) {
...
void MyClient::GetAllPropertyName(INDI::BaseDevice *dp) {
    ...
    for (const auto &property : dp->getProperties()) {
        ...
        qDebug() << "\033[31m" << propertyName << "\033[0m" << "\033[32m"
                 << propertyType << "\033[0m";
    }
}
...
uint32_t MyClient::getCCDFrameInfo(INDI::BaseDevice *dp, int &X, int &Y,
...
uint32_t MyClient::setCCDFrameInfo(INDI::BaseDevice *dp, int X, int Y,
                                   int WIDTH, int HEIGHT) {
    INDI::PropertyNumber ccdFrameInfo = dp->getProperty("CCD_FRAME");
...
uint32_t MyClient::getCCDBasicInfo(INDI::BaseDevice *dp, int &maxX, int &maxY,
                                   double &pixelsize, double &pixelsizX,
                                   double &pixelsizY, int &bitDepth) {
    ...
    qDebug() << "getCCDBasicInfo" << maxX << maxY << pixelsize << pixelsizX
             << pixelsizY << bitDepth;
    return QHYCCD_SUCCESS;
}

uint32_t MyClient::getCCDBinning(INDI::BaseDevice *dp, int &BINX, int &BINY,
                                 int &BINXMAX, int &BINYMAX) {
...
uint32_t MyClient::getCCDCFA(INDI::BaseDevice *dp, int &offsetX, int &offsetY,
                             QString &CFATYPE) {
...
uint32_t MyClient::getCCDGain(INDI::BaseDevice *dp, int &value, int &min,
                              int &max) {
...
uint32_t MyClient::getCCDOffset(INDI::BaseDevice *dp, int &value, int &min,
                                int &max) {
...
uint32_t MyClient::getCCDReadMode(INDI::BaseDevice *dp, int &value, int &min,
                                  int &max) {
...
uint32_t MyClient::StartWatch(INDI::BaseDevice *dp) {
    ...
    watchDevice(dp->getDeviceName(), [this](INDI::BaseDevice device) {
        ...
        device.watchProperty("CCD_TEMPERATURE", [this](INDI::PropertyNumber
                                                           property) {
            // if (dp->isConnected())
            // {
            ...
            property.onUpdate([property, this]() {
                ...
                if (property[0].getValue() == -20) {
                    qDebug(
                        "Watching | CCD temperature reached desired value!\n");
                }
            });
        });
        ...
        device.watchProperty("CCD1", [](INDI::PropertyBlob property) {
            ...
            property.onUpdate([property]() {
                ...
                qDebug(
                    "Watching | Received image, saved as ccd_simulator.fits\n");
            });
        });
    });
...
**                                  Mount API
***************************************************************************************/
uint32_t MyClient::getTelescopeInfo(INDI::BaseDevice *dp,
                                    double &telescope_aperture,
                                    double &telescope_focal,
                                    double &guider_aperature,
                                    double &guider_focal) {
    ...
    qDebug() << "getTelescopeInfo" << telescope_aperture << telescope_focal
             << guider_aperature << guider_focal;
    return QHYCCD_SUCCESS;
}

uint32_t MyClient::setTelescopeInfo(INDI::BaseDevice *dp,
                                    double telescope_aperture,
                                    double telescope_focal,
                                    double guider_aperature,
                                    double guider_focal) {
...
uint32_t MyClient::setTelescopeTrackEnable(INDI::BaseDevice *dp, bool enable) {
    ...
    if(enable==true){
     while(t.elapsed()<6000){
        ...
        QThread::msleep(100);
        if(property->getState()==IPS_IDLE) break;  //when enabled, it will
    become busy
     }
    }
    else{
        while(t.elapsed()<6000){
           qDebug()<<property->getStateAsString();
           QThread::msleep(100);
           if(property->getState()==IPS_IDLE) break;  //when disabled, it will
    become idle
     }
    }
...
uint32_t MyClient::setTelescopeParkOption(INDI::BaseDevice *dp,
                                          QString option) {
...
uint32_t MyClient::getTelescopeParkPosition(INDI::BaseDevice *dp,
                                            double &RA_DEGREE,
                                            double &DEC_DEGREE) {
...
uint32_t MyClient::setTelescopeParkPosition(INDI::BaseDevice *dp,
                                            double RA_DEGREE,
                                            double DEC_DEGREE) {
    INDI::PropertyNumber property = dp->getProperty("TELESCOPE_PARK_POSITION");
...
uint32_t MyClient::getTelescopeSlewRate(INDI::BaseDevice *dp, int &speed) {
    ...
    if (property[0].getState() == ISS_ON)
        ...
        speed = 3;
    else if (property[4].getState() == ISS_ON)
        speed = 4;
    else if (property[5].getState() == ISS_ON)
        speed = 5;
    else if (property[6].getState() == ISS_ON)
        speed = 6;
    else if (property[7].getState() == ISS_ON)
        speed = 7;
    else if (property[8].getState() == ISS_ON)
        speed = 8;
    else if (property[9].getState() == ISS_ON)
        speed = 9;

    qDebug() << "getTelescopeSlewRate" << speed;
    if (speed >= 0 && speed <= 9)
        qDebug() << property[speed].getLabel();
...
uint32_t MyClient::getTelescopeMaxSlewRateOptions(INDI::BaseDevice *dp,
                                                  int &min, int &max,
                                                  int &value) {
    //?? maybe onstep only
...
uint32_t MyClient::setTelescopeMaxSlewRateOptions(INDI::BaseDevice *dp,
                                                  int value) {
    //?? maybe onstep only
    INDI::PropertyNumber property = dp->getProperty("Max slew Rate");
...
uint32_t MyClient::setTelescopeGuideNS(INDI::BaseDevice *dp, int dir,
                                       int time_guide) {
...
uint32_t MyClient::setTelescopeGuideWE(INDI::BaseDevice *dp, int dir,
                                       int time_guide) {
...
uint32_t MyClient::setTelescopeActionAfterPositionSet(INDI::BaseDevice *dp,
                                                      QString action) {
...
uint32_t MyClient::getTelescopeRADECJ2000(INDI::BaseDevice *dp,
                                          double &RA_Hours,
                                          double &DEC_Degree) {
...
uint32_t MyClient::setTelescopeRADECJ2000(INDI::BaseDevice *dp, double RA_Hours,
                                          double DEC_Degree) {
    INDI::PropertyNumber property = dp->getProperty("EQUATORIAL_COORD");
...
uint32_t MyClient::getTelescopeRADECJNOW(INDI::BaseDevice *dp, double &RA_Hours,
                                         double &DEC_Degree) {
...
uint32_t MyClient::setTelescopeRADECJNOW(INDI::BaseDevice *dp, double RA_Hours,
                                         double DEC_Degree,
    ...
    //     qDebug()<<property->getStateAsString();
    //     QThread::msleep(100);
    //     if(property->getState()==IPS_IDLE) break;  // it will not wait the
    //     motor arrived
    // }
...
uint32_t MyClient::getTelescopeTargetRADECJNOW(INDI::BaseDevice *dp,
                                               double &RA_Hours,
                                               double &DEC_Degree) {
...
uint32_t MyClient::setTelescopeTargetRADECJNOW(INDI::BaseDevice *dp,
                                               double RA_Hours,
                                               double DEC_Degree) {
    INDI::PropertyNumber property = dp->getProperty("TARGET_EOD_COORD");
...
uint32_t MyClient::slewTelescopeJNowNonBlock(INDI::BaseDevice *dp,
                                             double RA_Hours, double DEC_Degree,
                                             bool EnableTracking,
                                             INDI::PropertyNumber &property) {
    QString action;
...
uint32_t MyClient::syncTelescopeJNow(INDI::BaseDevice *dp, double RA_Hours,
                                     double DEC_Degree,
                                     INDI::PropertyNumber &property) {
...
uint32_t MyClient::getTelescopetAZALT(INDI::BaseDevice *dp, double &AZ_DEGREE,
                                      double &ALT_DEGREE) {
...
uint32_t MyClient::setTelescopetAZALT(INDI::BaseDevice *dp, double AZ_DEGREE,
                                      double ALT_DEGREE) {
    INDI::PropertyNumber property = dp->getProperty("HORIZONTAL_COORD");
...
**                                  Focus API
***************************************************************************************/
uint32_t MyClient::getFocuserSpeed(INDI::BaseDevice *dp, int &value, int &min,
                                   int &max) {
...
uint32_t MyClient::getFocuserMoveDiretion(INDI::BaseDevice *dp,
                                          bool &isDirectionIn) {
    ...
    qDebug() << "getFocuserMoveDiretion | IN/OUT isDirectionIn:"
             << isDirectionIn;
    return QHYCCD_SUCCESS;
}

uint32_t MyClient::setFocuserMoveDiretion(INDI::BaseDevice *dp,
                                          bool isDirectionIn) {
    INDI::PropertySwitch property = dp->getProperty("FOCUS_MOTION");
    ...
    sendNewProperty(property);
    // qDebug() << "setFocuserMoveDiretion | IN/OUT isDirectionIn:" <<
    // isDirectionIn ;
    return QHYCCD_SUCCESS;
}
...
uint32_t MyClient::moveFocuserToAbsolutePosition(INDI::BaseDevice *dp,
                                                 int position) {
...
uint32_t MyClient::getFocuserAbsolutePosition(INDI::BaseDevice *dp,
                                              int &position) {
    INDI::PropertyNumber property = dp->getProperty("ABS_FOCUS_POSITION");
...
uint32_t MyClient::moveFocuserWithTime(INDI::BaseDevice *dp, int msec) {
    // move the focuser at defined motion direction and defined move speed with
    // msec time
...
uint32_t MyClient::getFocuserOutTemperature(INDI::BaseDevice *dp,
                                            double &value) {
...
uint32_t MyClient::getFocuserChipTemperature(INDI::BaseDevice *dp,
                                             double &value) {
...
**                                  CFW API
***************************************************************************************/
uint32_t MyClient::getCFWPosition(INDI::BaseDevice *dp, int &position, int &min,
                                  int &max) {
...
**                         Generic Properties
***************************************************************************************/
uint32_t MyClient::getDevicePort(INDI::BaseDevice *dp,
                                 QString &Device_Port)  // add by CJQ 2023.3.3
{
...
uint32_t MyClient::setDevicePort(INDI::BaseDevice *dp,
                                 QString Device_Port)  // add by CJQ 2023.3.28
{
    INDI::PropertyText property = dp->getProperty("DEVICE_PORT");
...
uint32_t MyClient::setTimeUTC(INDI::BaseDevice *dp, QDateTime datetime) {
    ...
    qDebug() << "setTimeUTC | Time zone offset:"
             << timeZone.offsetFromUtc(datetime);
    qDebug() << "setTimeUTC | Time zone abbreviation:"
             << timeZone.abbreviation(datetime);
    ...
    qDebug() << "property[0].setText(time_utc.toLatin1().data());"
             << time_utc.toLatin1().data();
    qDebug() << "property[1].setText(offset.toLatin1().data());"
             << offset.toLatin1().data();
...
uint32_t MyClient::setLocation(INDI::BaseDevice *dp, double latitude_degree,
                               double longitude_degree, double elevation) {
    ...
    qDebug() << "setLocation" << latitude_degree << longitude_degree
             << elevation;
    return QHYCCD_SUCCESS;
}

uint32_t MyClient::getLocation(INDI::BaseDevice *dp, double &latitude_degree,
                               double &longitude_degree, double &elevation) {
    INDI::PropertyNumber property = dp->getProperty("GEOGRAPHIC_COORD");
    ...
    qDebug() << "getLocation" << latitude_degree << longitude_degree
             << elevation;
...
uint32_t MyClient::setAtmosphere(INDI::BaseDevice *dp, double temperature,
...
uint32_t MyClient::getAtmosphere(INDI::BaseDevice *dp, double &temperature,
                                 double &pressure, double &humidity) {
    INDI::PropertyNumber property = dp->getProperty("ATMOSPHERE");
